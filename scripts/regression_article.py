...
profiles from ``scripts.pitcher_slate_analysis`` and the hitter profiles from
``scripts.comprehensive_report`` — rendered as one document in prose: arms
first, then bats.
...
def build_html(
    ...
    pmap = psa._pitcher_id_map(preds)
    bmap = cr._batter_id_map(preds)
    ...
    def arms(title: str, intro: str, rows: list, positive: bool) -> str:
        entries = "".join(
            _pitcher_entry(
                p,
                pctxs.get(p["name"]),
                psa._bets_for(pmap.get(p["name"], -1), preds),
                positive,
            )
            for p in rows
        )
    ...
    def bats(title: str, intro: str, rows: list, positive: bool) -> str:
        entries = "".join(
            _batter_entry(
                p,
                bctxs.get(p["name"]),
                cr._best_batter_bet(bmap.get(p["name"], -1), preds),
                positive,
            )
            for p in rows
        )
