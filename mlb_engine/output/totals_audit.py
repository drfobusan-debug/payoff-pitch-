...
def summarize(rows: list[LedgerRow]) -> Summary:
    ...
    return Summary(sign, rank_o, rank_u, base, by_bucket, len({r.date for r in graded}), len(graded))
...
def write_workbook(path: Path, sheet_day: Date, yesterday: list[LedgerRow], ledger: list[LedgerRow]) -> Path:
    ...
    for name, t in total.by_bucket.items():
        rec(name, t)
    s.append([])
    s.append(["Break-even at -110 is 52.4%. Sign and rank rates are graded against the sheet's own line at write time."])
