...
WEIGHTS = {"bat vs hand": 1.0, "bp": 1.5, "sp": 1.0}
WEIGHTS_VERSION = "w1-1.5-1-dec-spz"
N_DECILES = 10
...
def grade_pending(rows: list[LedgerRow], today: Date) -> int:
    days = sorted({
        ...
        if not r.graded and r.complete and Date.fromisoformat(r.date) < today
    })
    n = 0
    for d in days:
        try:
...
def write_workbook(
    ...
    ws.append([
        ...
        f"{SP_MIN_IP:.0f} IP; * = a starter is missing so the total is incomplete and the game is neither graded nor audited). "
