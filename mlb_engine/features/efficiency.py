...
class PitcherEfficiency:
    ...
    def gb_dp_rate(self) -> float:
        """P(two outs | batted-ball out, force at second) for this pitcher.

        Measured, not scaled off an assumed anchor: a ground-ball out with the
        force on turns two 39.1% of the time and an air out 4.8%, so a pitcher's
        rate follows from his GB%. The old 12% anchor was well below the 21.4%
        the league actually turns.
        """
